...
let lastTcpStatus = 'starting TCP bridge…';

const isDev = !app.isPackaged;

const sendTcpStatus = (status) => {
  lastTcpStatus = status;
  if (mainWindow) {
    mainWindow.webContents.send('tcp-status', status);
  }
};
...
const startTCPServer = () => {
  const server = createServer((socket) => {
    ...
    const remoteLabel = [socket.remoteAddress, socket.remotePort]
      ...
      .join(':') || 'unknown';
    sendTcpStatus(`client connected (${remoteLabel}) · ${tcpClients.size} active`);
    let buffer = '';
    ...
    socket.on('close', () => {
      if (tcpClients.has(socket)) {
        tcpClients.delete(socket);
        sendTcpStatus(`client disconnected (${remoteLabel}) · ${tcpClients.size} active`);
      }
    });
    ...
    socket.on('error', (error) => {
      ...
      if (tcpClients.has(socket)) {
        tcpClients.delete(socket);
        sendTcpStatus(`client error (${remoteLabel}) · ${tcpClients.size} active`);
      }
    });
  });
  ...
  server.listen(port, () => {
    ...
    sendTcpStatus(message);
  });
  server.on('error', (error) => {
    console.error('TCP server error', error);
    sendTcpStatus(`server error: ${error.message}`);
  });
};
