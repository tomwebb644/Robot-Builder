...
const InspectorPanel: React.FC = () => {
  ...
  const handleRemove = () => {
    ...
    const confirmed = window.confirm(
      `Delete “${node.name}” and ${node.children.length} linked child${node.children.length === 1 ? '' : 'ren'}?`
    );
    if (confirmed) {
      removeNode(node.id);
    }
      ...
      <div className="panel-section inspector-panel">
        ...
        {!isRoot ? (
          <section className="inspector-actions">
            ...
            <p className="hint">Removing a link also removes all of its descendants.</p>
          </section>
        ) : null}
      </div>
    </div>
