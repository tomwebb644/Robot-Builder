...
- **Hierarchy Editing** – Delete any non-root link from the inspector; descendants are cleaned up automatically.
...
- **Persistence** – Save the entire scene graph to disk or reload a JSON project using native OS dialogs. When running the Vite dev server in a browser, the app falls back to in-browser downloads/uploads.
...
If a joint has its "Enable TCP control" checkbox enabled, the incoming value is clamped to the configured limits and applied instantly. Remote updates also refresh the TCP status indicator, which shows connection, disconnect, or error events from the TCP server.
